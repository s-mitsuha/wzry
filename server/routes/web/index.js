module.exports = app => {
    ...
    router.get('/init/news',async(req,res)=>{
        const newsArrs = ["2月3日净化游戏环境声明及处罚公告", "2月3日外挂专项打击公告", "2月3日“演员”惩罚名单", "2月2日体验服停机更新公告", "公孙离惊鸿舞表演来了！不夜长安上元夺魁时，一起欣赏阿离姐姐绝美舞姿！", "公孙离惊鸿舞表演来了！不夜长安上元夺魁时，一起欣赏阿离姐姐绝美舞姿！", "嫦娥皮肤设计第9期新创意赏析！烟岚云岫、水调歌头、越窑清灵...", "王者荣耀携手麦斯威尔，开启新年敦煌送福大赏！", "参观嫦娥姐姐的创意工坊！欣赏新衣得钻石奖励~", "CHALI茶里×王者荣耀 | 跨界出击，给新年加buff！", "2月3日净化游戏环境声明及处罚公告", "2月3日外挂专项打击公告", "2月3日“演员”惩罚名单", "2月3日全服不停机更新公告", "2月2日体验服停机更新公告", "【微信用户专属】“游戏礼品站”购买“小乔-青蛇”、“阿轲-节奏热浪”抽免单活动", "积分暴暴暴活动开启，最高可暴击1000荣耀积分", "芈月-白晶晶皮肤即将登场，限时语音包同步上线", "【上元夺魁·集群英筹庆典】活动开启公告", "镜-炽阳神光 FMVP定制皮肤即将登场，应援冬冠参与活动得好礼", "2021年KPL春季转会期俱乐部挂牌名单", "2021KPL春季转会期挂牌期第三阶段追加公告", "关于2021年KPL春季赛临时参赛资格评审结果的通知", "2021年KPL王者荣耀职业联赛赛制升级，全新对抗一触即发！", "英雄归来，共度荣耀！2020王者荣耀冬季冠军杯暨年度颁奖典礼圆满落幕"]
        const newsCatID = await Category.findOne({
            name: '新闻资讯'
        })
        const categories = await Category.find({
            parent: newsCatID._id
        })
        // 构建要插入的数据
        const articles = newsArrs.map(el =>{
            return {
                title: el,
                // random [0.0,1.0)*5 = [0.0,5.0)
                // floor [0.0,5.0) = [0.0,1.0)=>0 [1.0,2.0)=>1 
                // ceil[0.0,5.0) = [0.0,1.0)=>1 [1.0,2.0)=>2 ... [4.0,5.0)=>5
                // round[0.0,5.0) = [0.0,0.5)=>0 [0.5,1.5)=>1... [4.5,5.0)=>5
                categories: new Array(1).fill(categories[Math.ceil(Math.random() * categories.length-1)]._id)
            }
        })
        // 清空Article数据
        const delRes = await Article.deleteMany({})
        // 往Article插入多条数据
        const insertRes = await Article.insertMany(articles)
        res.send(articles)
    })
    ...
    router.get('/init/heroes',async(req,res)=>{
        ...
        for(let cat of heroesArrs){
            ...
            // 往Hero插入多条数据
            const insertRes = await Hero.insertMany(cat.heroes)
        }
    ...
    router.get('/heroes/list',async(req,res)=>{
        const heroesCatID = await Category.findOne({
            name: '英雄列表'
        })
        const categories = await Category.find({
            parent: heroesCatID._id
        })
        let result = []
        for(let i = 0 ; i < categories.length ; i++){            
            result[i] = {
                name: categories[i].name,
                heroes: await  Hero.find({categories: new Array(1).fill(categories[i]._id)}).lean()
            }
        }
        result.unshift({name: '热门',heroes: await  Hero.find().limit(10).lean()})
        res.send(result)
    })

    router.get('/article/:id',async(req,res)=>{
        let article = await Article.findById(req.params.id).lean()
        article.relative = await Article.find({
    ...
    router.get('/hero/:id',async(req,res)=>{
        let hero = await Hero.findById(req.params.id)
          .populate('categories tailWindItems headWindItems partners.hero')
          .lean()
        res.send(hero)
    })
